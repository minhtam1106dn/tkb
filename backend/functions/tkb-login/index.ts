...
Deno.serve(async request => {
 ...
 try {
  ...
  const [profileResult,scheduleResult,taskResult,noteResult,catalogResult]=await Promise.all([
  ...
  const bootstrap=catalogResult.ok && scheduleResult.ok && (!taskResult || taskResult.ok) && (!noteResult || noteResult.ok)?{
   catalog:await catalogResult.json(),role,day:day||null,schedules:await scheduleResult.json(),tasks:taskResult?await taskResult.json():[],notes:noteResult?await noteResult.json():[]
  }:null;
