...
- Parents edit recurring weekly schedules in `schedule-editor.js`. `tkb_save_timetable_day` validates school periods, optional teachers, afternoon starting period and extra-class times; it updates only one weekday using the expected previous day to reject concurrent overwrites. Timetables remain read-only for children under RLS and refresh through the normal sync cycle. `test_timetable.sql` runs edits inside a rolled-back transaction.
