...
def call(method,params={}):
 if method.startswith('Input.'):call('Page.bringToFront')
...
# Regression: only the dialog scrolls; closing restores the page exactly.
click('[data-view="checklist"]')
for width in [375,393,430,768,1440]:
 call('Page.bringToFront')
 call('Emulation.setDeviceMetricsOverride',{'width':width,'height':812,'deviceScaleFactor':1,'mobile':width<700})
 ev("document.getElementById('manage-tasks').scrollIntoView({block:'center'});window.pageBefore={y:scrollY,top:document.getElementById('app').getBoundingClientRect().top}")
 click('#manage-tasks')
 check("getComputedStyle(document.body).position==='fixed'&&Math.abs(document.getElementById('app').getBoundingClientRect().top-pageBefore.top)<2",f'page position preserved on open {width}')
 coords=ev("(()=>{const r=document.getElementById('task-editor').getBoundingClientRect();return {x:r.x+r.width*.7,y:r.y+r.height*.7}})()")
 call('Input.dispatchMouseEvent',{'type':'mouseWheel','x':coords['x'],'y':coords['y'],'deltaX':0,'deltaY':400})
 time.sleep(.3)
 check("document.getElementById('task-editor').scrollTop>0&&Math.abs(document.getElementById('app').getBoundingClientRect().top-pageBefore.top)<2",f'wheel scroll stays in popup {width}')
 ev("document.getElementById('task-editor').scrollTop=99999")
 call('Input.dispatchMouseEvent',{'type':'mouseWheel','x':coords['x'],'y':coords['y'],'deltaX':0,'deltaY':500})
 call('Input.dispatchMouseEvent',{'type':'mouseWheel','x':2,'y':400,'deltaX':0,'deltaY':500})
 time.sleep(.2)
 check("Math.abs(document.getElementById('app').getBoundingClientRect().top-pageBefore.top)<2",f'bottom and backdrop do not scroll page {width}')
 if width==375:
  ev("document.getElementById('task-editor').scrollTop=0")
  call('Page.bringToFront')
  ...
  for distance in [40,80,140]:
   call('Input.dispatchTouchEvent',{'type':'touchMove','touchPoints':[{'x':coords['x'],'y':coords['y']-distance,'id':1}]});time.sleep(.05)
  call('Input.dispatchTouchEvent',{'type':'touchEnd','touchPoints':[]});time.sleep(.4)
  check("document.getElementById('task-editor').scrollTop>0&&Math.abs(document.getElementById('app').getBoundingClientRect().top-pageBefore.top)<2",'touch scroll only moves popup')
  call('Emulation.setTouchEmulationEnabled',{'enabled':False})
 click('#catalog-close')
 check("!document.body.classList.contains('task-editor-open')&&Math.abs(scrollY-pageBefore.y)<2",f'close restores scroll {width}')
 click('#manage-tasks')
 call('Input.dispatchKeyEvent',{'type':'keyDown','key':'Escape','code':'Escape','windowsVirtualKeyCode':27})
 call('Input.dispatchKeyEvent',{'type':'keyUp','key':'Escape','code':'Escape','windowsVirtualKeyCode':27});time.sleep(.15)
 check("!document.getElementById('task-editor').open&&!document.body.classList.contains('task-editor-open')&&Math.abs(scrollY-pageBefore.y)<2",f'Escape restores scroll {width}')
call('Emulation.setDeviceMetricsOverride',{'width':375,'height':812,'deviceScaleFactor':1,'mobile':False})
click('#manage-tasks')
ev("document.getElementById('catalog-close').click();document.getElementById('manage-tasks').click()");time.sleep(.2)
check("document.getElementById('task-editor').open&&document.body.classList.contains('task-editor-open')",'rapid reopen retains scroll lock')
# End modal scroll regression.
